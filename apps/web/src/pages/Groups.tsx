...
// 扩展类型用于处理 topics
type GroupWithTopics = GroupWithStats & {
  topics?: GroupWithStats[];
  isVirtual?: boolean;
};
...
export function Groups() {
  ...
  // 按账号分组群组（树形结构）
  const groupsByAccount: AccountWithGroups[] = accounts.map((account: Account) => {
    ...
    const groupsWithTopics = parentGroups.map(
      (group: GroupWithStats): GroupWithTopics => ({
        ...group,
        topics: topics.filter((t: GroupWithStats) => t.telegramId === group.telegramId),
      }),
    );
    ...
    const orphanGroupMap = new Map<string, GroupWithStats[]>();
    orphanTopics.forEach((topic: GroupWithStats) => {
    ...
    const virtualParentGroups = Array.from(orphanGroupMap.entries()).map(
      ([telegramId, topicList]): GroupWithTopics => {
    ...
    <div>
      ...
      {isLoading || isLoadingAccounts ? (
        ...
        <div className="mt-6 space-y-4">
          {groupsByAccount.map(({ account, groups: accountGroups }: AccountWithGroups) => (
            <div key={account.id} className="bg-white shadow sm:rounded-lg overflow-hidden">
              ...
              {expandedAccounts.has(account.id) && (
                <ul className="divide-y divide-gray-200">
                  {accountGroups.map((group: GroupWithTopics) => (
                    <li key={group.id}>
                      {/* 父 Group */}
                        ...
                        {expandedGroups.has(group.id) &&
                          ...
                          group.topics.length > 0 && (
                            <div className="ml-6 mt-2 space-y-2 border-l-2 border-gray-200 pl-4">
                              {group.topics.map((topic: GroupWithStats) => (
                                  ...
                                  <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                      ...
                                      <div className="mt-1 flex items-center text-xs text-gray-500">
                                        {topic.topicId && (
                                          <span className="mr-4">Topic ID: {topic.topicId}</span>
                                        )}
                                        <span className="mr-4">
